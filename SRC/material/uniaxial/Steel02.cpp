...
)
{
    ...
    // Derivative of xi
    double depsplDtheta = 0.0;
    bool onInitialCurveForXi = (fabs(epsr) < 1e-15 && fabs(sigr) < 1e-15);
    if (onInitialCurveForXi) {
        depsplDtheta = epss0Sens;
    }

    double xiSign = (epspl - epss0) >= 0 ? 1.0 : -1.0;
    double dxiDtheta = xiSign * ((depsplDtheta - epss0Sens) * epsy -
    ...
    double gradient = df_starDtheta * (sigs0 - sigr) +
